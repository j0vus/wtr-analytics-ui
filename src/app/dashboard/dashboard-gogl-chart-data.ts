...
export class DashBoardGOGChartData extends AppComponentBase {
      ...
      .subscribe((res: any) => {
        ...
        if (res.body && res.body.length > 0) {
          if (
            res.body.length == 10
              ? res.body[4].value
              : res.body[res.body.length - 1].value > 3600
          ) {
            ...
            this.industryTimeChartData.forEach((item) => {
              ...
              data.push([item.key, parseFloat(num.toFixed(2))]);
            });
  ...
  initMainChart(uri: string) {
    this.apiService
      .get(AppConsts.uniqueVisitors + uri)
      .subscribe((res: any) => {
        ...
        if (
          this.mainChartVisitorData.length > 30 &&
          this.mainChartVisitorData.length <= 90
        ) {
          ...
          this.mainChartVisitorData.forEach((item) => {
            ...
            const weekRange = `${startDate.getDate()} - ${endDate.getDate()} ${endDate.toLocaleString(
              "en-US",
              { month: "short" }
            )} `;
          ...
          let weeklySumData = Array.from(weeklyData, ([weekRange, value]) => ({
            weekRange,
            value,
          }));

          weeklySumData.forEach((item) => {
            data.push([item.weekRange, item.value]);
          });
          ...
          this.mainChartVisitorData.forEach((item) => {
            ...
            monthlyData.set(
              monthYearString,
              monthlyData.get(monthYearString) + item.value
            );
          });

          let monthlySumData = Array.from(
            monthlyData,
            ([monthYearString, value]) => ({ monthYearString, value })
          );

          monthlySumData.forEach((item) => {
            data.push([item.monthYearString, item.value]);
          });
        } else {
          ...
          this.mainChartVisitorData.forEach((item) => {
            const date = new Date(item.key);
            const dayDateStr = date.toLocaleString("en-US", {
              month: "short",
              day: "numeric",
            });
          ...
          let dailySumData = Array.from(dailyData, ([dayDateStr, value]) => ({
            dayDateStr,
            value,
          }));

          dailySumData.forEach((item) => {
            data.push([item.dayDateStr, item.value]);
          });
        }
